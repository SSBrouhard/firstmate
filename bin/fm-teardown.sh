...
codex_app_pr_landed() {
  local state base merge default target
  [ -n "$PR_URL" ] || return 1
  command -v gh >/dev/null 2>&1 || return 1

  state=$(gh pr view "$PR_URL" --json state -q .state 2>/dev/null || true)
  [ "$state" = MERGED ] || return 1

  base=$(gh pr view "$PR_URL" --json baseRefName -q .baseRefName 2>/dev/null || true)
  merge=$(gh pr view "$PR_URL" --json mergeCommit -q .mergeCommit.oid 2>/dev/null || true)
...
if [ "$BACKEND" = codex-app ] && [ "$KIND" != scout ] && [ "$FORCE" != "--force" ]; then
  ...
  if [ -z "$WT" ]; then
    ...
    codex_dirty=$(git -C "$WT" status --porcelain 2>/dev/null | head -1 || true)
    codex_unpushed=$(git -C "$WT" log --oneline HEAD --not --remotes -- 2>/dev/null | head -5 || true)
    if [ -n "$codex_dirty" ] || { [ -n "$codex_unpushed" ] && ! work_is_landed "$branch"; }; then
      echo "REFUSED: worktree $WT has work not on any remote and not landed." >&2
      [ -n "$codex_dirty" ] && echo "uncommitted changes present" >&2
      [ -n "$codex_unpushed" ] && printf 'unpushed commits:\n%s\n' "$codex_unpushed" >&2
...
rm -f "$STATE/$ID.codex-app.env" "$STATE/$ID.codex-app.log" "$STATE/$ID.codex-app.capture" "$STATE/$ID.codex-app-send."*
