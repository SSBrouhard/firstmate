...
Primitive-only Orca terminal operations require the `orca` CLI only when `backend=orca` is selected, but Orca cannot spawn tasks yet.
...
`orca` currently exposes terminal primitives only.
