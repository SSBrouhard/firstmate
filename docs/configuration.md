...
`orca` is a known primitive-only adapter for already-created Orca terminals: capture, text send, Enter/interrupt keys, and close are wired, but task spawning is refused until its lifecycle wiring exists.
