...
For `codex-app`, the Codex Desktop thread tools are the ground truth; `fm-peek` can show only cached captures, `fm-send` refuses with the host-tool action to take, and `fm-teardown` refuses until app archive and landed-work safety are recorded.
